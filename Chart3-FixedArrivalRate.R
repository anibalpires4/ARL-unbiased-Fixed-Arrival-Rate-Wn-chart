...
pdf.VU1 <- function(x, lambda, mu, k) as.numeric(x<=0)*lambda*cdf.VU1(x,lambda,mu, k) + as.numeric(x>0)*( dgamma(x, k, k*mu) + lambda*exp(lambda*x)*(k*mu/(k*mu+lambda))^k * ( 1 - pgamma(x, k, k*mu+lambda) ) - exp(lambda*x)*(k*mu/(k*mu+lambda))^k*dgamma(x, k, k*mu+lambda) )
...
BE.arl <- function(h, gamma, lambda, mu, k, r=20) {
  ii <- 1:(r-1)
  w <- 2*h/(2*r-1)  
...
BE2.arl <- function(h, gamma, lambda, mu, k, r=250) {
  ...
  I <- diag(1, r+1)
  ARL <- solve(I-Q, one)  
  arl <- 1 + (1-gamma)*cdf.VU1(0, lambda, mu, k)*ARL[1] + sum( Qj(ii) * ARL[-1] )
  #arl <- ARL[1]
  arl
}
...
get.gammaAh.lambda <- function(L0, lambda, mu, k, z0=0, r=40, qm=30, eps=1e-6, OUTPUT=FALSE) {
  ...
  h.error <- 1
  dr.error <- 1
  while ( abs(h.error) > 1e-8 & abs(dr.error) > 1e-8 ) {
...
# CDF and PDF of V-U
perl <- Vectorize(function(x, k, lambda) {
  kk <- 0:(k-1)
  result <- 0
  if ( x > 0 ) result <- 1 - exp(-lambda*x) * sum( (lambda*x)^kk / factorial(kk) )
  result
})

#k <- 2
cdf.VU1 <- Vectorize(function(x, k, lambda, mu) {
  if ( x<=0 ) result <- 1-perl(-x, k, k*lambda) - ( k*lambda/(k*lambda+mu) )^k * sum( (-(k*lambda+mu)*x)^(0:(k-1)) / factorial(0:(k-1)) ) * exp(k*lambda*x)
  if ( x>0 ) result <- 1 - exp(-mu*x)*(k*lambda/(k*lambda+mu))^k
  result
})

pdf.VU1 <- Vectorize(function(x, k, lambda, mu) as.numeric(x<=0)*( dgamma(-x, k, k*lambda) + mu*( k*lambda/(k*lambda+mu) )^k * sum( (-(k*lambda+mu)*x)^(0:(k-1)) / factorial(0:(k-1)) ) * exp(k*lambda*x) - (k*lambda)^k*(-x)^(k-1)*exp(k*lambda*x)/factorial(k-1) ) + as.numeric(x>0)*mu*exp(-mu*x)*(k*lambda/(k*lambda+mu))^k)

qf.VU1 <- function(y, k, lambda, mu) {
  ff <- function(x) y - cdf.VU1(x, k, lambda, mu)
  x <- uniroot(ff, c(-50, 50))$root
  x
}
...
BE.arl <- function(h, gamma, lambda, mu, k, r=20) {
  ii <- 1:(r-1)
  w <- 2*h/(2*r-1)  
  qij <- function(i,j) cdf.VU1( (j-i)*w+w/2, k, lambda, mu) - cdf.VU1( (j-i)*w-w/2, k, lambda, mu)
  Qi <- function(i)    cdf.VU1(   -i*w+w/2,  k,lambda, mu)
  Q <- rbind( cbind( Qi(0), t(qij(0,ii)) ), 
              cbind( Qi(ii), outer(ii,ii,qij) ) )
  Q[,1] <- (1-gamma) * Q[,1]
  one <- array(1, r)
...
BE2.arl <- function(h, gamma, lambda, mu, k, r=250) {
  ...
  w <- h/r  
  qij <- function(i,j) cdf.VU1( (j-i)*w+w/2, k, lambda, mu) - cdf.VU1( (j-i)*w-w/2, k, lambda, mu)
  Qi <- function(i)    cdf.VU1(-(i-1)*w-w/2, k, lambda, mu)
  Qj <- function(j)    cdf.VU1( j*w, k, lambda, mu) - cdf.VU1( (j-1)*w, k, lambda, mu)
  Q <- rbind( cbind( cdf.VU1(0, k, lambda, mu), t(Qj(ii)) ), 
              cbind( Qi(ii), outer(ii,ii,qij) ) )      
  Q[,1] <- (1-gamma) * Q[,1]
  one <- array(1, r+1)
  I <- diag(1, r+1)
...
coll.arl <- function(h, gamma, lambda, mu, k, z0=0, r=40, qm=30) {
  ...
  for ( i in 1:r ) {
    zi <- zch[i]
    xm1 <- zi/2
    h_ <- qf.VU1(1-1e-9, k, lambda, mu) + zi
    if ( h_ > h ) h_ <- h
    xm2 <- (zi+h_)/2
    xw1 <- zi/2
    xw2 <- (h_-zi)/2
    ...
    z2 <- xm2 + xw2 * z
    for ( j in 1:r ) {
      integral <- sum( w1 * pdf.VU1( z1-zi, k, lambda, mu) * Tn(2*z1/h-1, j-1)  ) + sum( w2 * pdf.VU1( z2-zi, k, lambda, mu) * Tn(2*z2/h-1, j-1)  )
      A[i,j] <- Tn(2*zi/h-1, j-1) - (1-gamma)*cdf.VU1(-zi, k, lambda, mu)*Tn(-1, j-1) - integral
    }
  }
  one <- rep(1, r) 
  g <- solve(A, one)
...
coll2.arl <- function(h, gamma, lambda, mu, k, z0=0, r=40, qm=30) {
  ...
  for ( i in 1:r ) {
    ...
    z2 <- xm2 + xw2 * z
    for ( j in 1:r ) {
      integral <- cdf.VU1(h-zi, k, lambda, mu) - cdf.VU1(-zi, k, lambda, mu)*(-1)^(j-1) - sum( w1 * cdf.VU1( z1-zi, k, lambda, mu) * dTn(2*z1/h-1, j-1)*2/h  ) - sum( w2 * cdf.VU1( z2-zi, k, lambda, mu) * dTn(2*z2/h-1, j-1)*2/h  )
      A[i,j] <- Tn(2*zi/h-1, j-1) - (1-gamma)*cdf.VU1(-zi, k, lambda, mu)*(-1)^(j-1) - integral
    }
  }
  one <- rep(1, r) 
  g <- solve(A, one)
...
# Define UI
ui <- fluidPage(
  titlePanel("ARL unbiased Wn chart - Fixed Arrival Rate"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        sliderInput("lambda", HTML("Target Traffic Intensity (&rho;0):"), min = 0.01, max = 0.99, value = 0.5, step = 0.01),
        sliderInput("L0", "Target ARL:", min = 100, max = 1000, value = 500, step = 1),
...
server <- function(input, output) {
  ...
  output$ARLPlot <- renderPlot({
    ...
    if (distribution == "M/Ek/1") {
      ...
      progress$inc(0.2, detail = "Initializing variables")
      ...
      progress$inc(0.3, detail = "Calculating limits")
      
      # Get h and gamma values for the specified L0, lambda, and mu
      calibration_results_m <- get.gammaAh.lambda_m(L0, lambda0, mu, k)
      h <- calibration_results_m$h
      gamma <- calibration_results_m$gamma
      
      progress$inc(0.3, detail = "Generating plot data")
      
      # lambda sequence for plotting
      lambda_seq <- seq(0.01, 0.99, by = 0.01)
      ARL_values <- sapply(lambda_seq, function(lambda) coll.arl_m(h, gamma, lambda, mu, k))
      
      progress$inc(0.2, detail = "Rendering plot")
      
      # Determine if the line comes into contact with the default legend position
      ...
      legend(legend_pos, legend = c( paste("UCL =", round(h, 4), "     "), paste("γL =", round(gamma, 6))), cex = 0.8, inset = c(0.025, 0.1))
